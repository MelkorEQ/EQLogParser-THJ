      ...
      try
      {
        ...
        if (split != null && split.Length >= 2)
        {
          ...
          else
          {
              ...
              if (!string.IsNullOrEmpty(split[i]))
              {
                switch (split[i])
                {
                  ...
                  case "non-melee":
                    ...
                    if( !found )
                    {
                        // ex. Founy Jestands was hit by non-melee for 31 points of damage
                        // does NOT work since we have to record whom the last attacker was!
                        if( lineData.Action.Contains( "was hit by non-melee for" ) )
                        {
                            dsIndex = i - 2;
                        }
                    }
                  ...
                  case "deliver":
                    if (stop == i +4 && split.Length > 5 && split[i + 2] == "critical" && split[i + 3] == "blast!")
                    {
                      LastCrit = new OldCritData { Attacker = PlayerManager.Instance.ReplacePlayer(split[0], split[0]), LineData = lineData };
                    }
                    break;
                  case "Crippling":
            ...
            if (dsIndex > -1 && pointsOfIndex > dsIndex && isIndex > -1 && isIndex < dsIndex && byIndex > isIndex && byIndex < dsIndex)
            {
            ...
            // Xan - handle old DS (setting dsIndex to 'hit'
            // ex. Founy Jestands was hit by non-melee for 31 points of damage
            else if (dsIndex > -1 && pointsOfIndex == dsIndex +5 && byIndex == dsIndex +1 )
            {
              string defender = string.Join( " ", split, 0, dsIndex -1 );
              uint damage = StatsUtil.ParseUInt( split[pointsOfIndex - 1] );
              CreateDamageRecord(lineData, split, stop, Labels.DS, defender, damage, Labels.DS, Labels.DS);
            }
              ...
              uint damage = StatsUtil.ParseUInt(split[pointsOfIndex - 1]);
                ...
                if( split[ split.Length -1 ].EndsWith( ")" ) )
                {
                  ...
                  for( ; openp > 0; openp-- )
                  {
                    ...
                    {
                        spellname   = string.Join( " ", split, openp, split.Length -openp ).Trim( new char[]{ '(', ')'} );
                        openp   = 0;
                    }
                  }
                  if( spellname != "" )
